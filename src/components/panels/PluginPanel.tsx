    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-grove-bg border border-grove-border rounded-xl max-w-lg w-full max-h-[80vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-grove-border">
          <h2 className="text-lg font-display text-grove-accent">Plugins</h2>
          <button
            onClick={onClose}
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-mono text-grove-text-secondary">
                        v{plugin.version}
                      </span>
                    </div>
                    {plugin.description && (
                    ...
                    <div className="flex gap-3 text-[10px] text-grove-text-secondary">
                      {plugin.actions_count > 0 && (
                        <span>{plugin.actions_count} actions</span>
                      )}
                      {plugin.blocks_count > 0 && (
                        <span>{plugin.blocks_count} blocks</span>
                      )}
                      {plugin.data_sources_count > 0 && (
                        <span>{plugin.data_sources_count} sources</span>
                      )}
                    </div>
                  </div>
                  <button
        ...
        <div className="px-6 py-3 border-t border-grove-border">
          <p className="text-[10px] text-grove-text-secondary">
            Plugins add actions, block types, and data sources to Grove.
            Changes take effect on next reasoning cycle.
          </p>
        </div>
      </div>
    </div>
